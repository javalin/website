/* http://prismjs.com/download.html?themes=prism-okaidia&languages=markup+css+clike+javascript+bash+ruby+java+kotlin+sql&plugins=toolbar+copy-to-clipboard */
...
// fix bug in kotlin highlighter
setTimeout(() => {
    document.querySelectorAll(".language-kotlin .keyword").forEach(token => {
        if (token.innerText === "get") {
            token.classList.toggle("keyword")
            token.classList.toggle("function")
        }
    });
}, 0);
